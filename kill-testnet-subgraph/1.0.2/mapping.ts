...
// Spawn cost updated to 20
const UNIT_PRICE_WEI = BigInt.fromI32(20).times(KILL_DECIMALS);
// Move cost updated to 100
const MOVE_COST_WEI = BigInt.fromI32(100).times(KILL_DECIMALS);
...
// Treasury BPS updated to 30%
const GLOBAL_CAP_BPS = BigInt.fromI32(3000); 
...
export function handleSpawned(event: SpawnedEvent): void {
  ...
  // Match contract cost logic: totalCost = amount * unitPrice
  ...
  // If stack was empty, set birth block to now
  if (aStack.birthBlock.equals(BigInt.fromI32(0))) aStack.birthBlock = event.params.birthBlock
  aStack.save()
}
...
export function handleMoved(event: MovedEvent): void {
  ...
  // Move cost is flat 100 KILL, not per unit
  ...
  // Logic Fix: Only update birth block if moving to an empty stack.
  // If moving into an occupied stack, the birthBlock is preserved in contract.
  ...
  if (aStackTo.birthBlock.equals(BigInt.fromI32(0))) {
...
export function handleKilled(event: KilledEvent): void {
  ...
  if (stack.totalStandardUnits.equals(BigInt.fromI32(0)) && stack.totalBoostedUnits.equals(BigInt.fromI32(0))) {
    stack.active = false
    stack.birthBlock = BigInt.fromI32(0) // Reset birth block when stack is cleared
  }
...
export function handleGlobalStats(event: GlobalStatsEvent): void {
  ...
  // Current Treasury represents the net balance in the contract
  ...
  // Max bounty is capped by 30% of the current treasury
