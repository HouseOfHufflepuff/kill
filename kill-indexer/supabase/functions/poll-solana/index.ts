...
function parseEvents(logs: string[]): AnchorEvent[] {
  ...
  for (const log of logs) {
    ...
    if (disc.every((v, i) => v === DISC.StackSpawned[i])) {
      ...
      const d = raw.slice(8);
      events.push({ name: "KillEvent", attacker: pubkey(d, 0), defender: pubkey(d, 32), attackerStack: u16(d, 64), defenderStack: u16(d, 66), attackerBounty: u64(d, 68), defenderBounty: u64(d, 76), totalBurned: u64(d, 84), remainingUnits: u64(d, 92), remainingReapers: u64(d, 100), slot: u64(d, 108), attackerUnitsSent: u64(d, 116), attackerReapersSent: u64(d, 124), attackerUnitsLost: u64(d, 132), attackerReapersLost: u64(d, 140), defenderUnits: u64(d, 148), defenderReapers: u64(d, 156), defenderUnitsLost: u64(d, 164), defenderReapersLost: u64(d, 172) });
    }
  }
  return events;
}
...
// ─── DB helpers (same as helius-webhook) ──────────────────────────────────────
// deno-lint-ignore no-explicit-any
async function updateGlobalStat(db: any, delta: { units_killed?: bigint; reapers_killed?: bigint; kill_added?: bigint; kill_extracted?: bigint; kill_burned?: bigint }) {
  const { data: existing } = await db.from("global_stat").select("*").eq("id", "current").maybeSingle();
  const cur = existing ?? { total_units_killed: "0", total_reaper_killed: "0", kill_added: "0", kill_extracted: "0", kill_burned: "0", total_pnl: "0", current_treasury: "0", max_bounty: "0" };
  ...
  await db.from("global_stat").upsert({ id: "current", total_units_killed: n(unitsKilled), total_reaper_killed: n(reapKilled), kill_added: n(killAdded), kill_extracted: n(killExtracted), kill_burned: n(killBurned), current_treasury: n(treasury > 0n ? treasury : 0n), total_pnl: n(killExtracted), max_bounty: "0" });
}
...
async function handleKillEvent(db: any, sig: string, idx: number, e: Extract<AnchorEvent, { name: "KillEvent" }>) {
  const id = `${sig}-${idx}`;
  await db.from("killed").upsert({ id, attacker: e.attacker, target: e.defender, stack_id: e.defenderStack, attacker_units_sent: n(e.attackerUnitsSent), attacker_reaper_sent: n(e.attackerReapersSent), attacker_units_lost: n(e.attackerUnitsLost), attacker_reaper_lost: n(e.attackerReapersLost), target_units_lost: n(e.defenderUnitsLost), target_reaper_lost: n(e.defenderReapersLost), initial_defender_units: n(e.defenderUnits), initial_defender_reaper: n(e.defenderReapers), attacker_bounty: n(e.attackerBounty), defender_bounty: n(e.defenderBounty), total_burned: n(e.totalBurned), target_birth_slot: 0, slot: Number(e.slot) });
  ...
  await updateGlobalStat(db, { units_killed: e.defenderUnitsLost, reapers_killed: e.defenderReapersLost, kill_extracted: e.attackerBounty + e.defenderBounty, kill_burned: e.totalBurned });
}
...
async function poll() {
  ...
  // Process oldest first, skip failed txs
  const toProcess = [...sigs].reverse().filter((s: { err: unknown }) => !s.err);
  let processed = 0;

  for (const { signature: sig } of toProcess) {
  ...
  // Advance checkpoint to most recent signature
  ...
  return { processed, latest: sigs[0].signature };
}

// ─── HTTP handler (triggered by pg_cron or manually) ─────────────────────────
